...
def _conv2d_relu(vgg_layers, prev_layer, layer, layer_name):
    ...
    with tf.variable_scope(layer_name) as scope:
        ...
        h_conv2d = tf.nn.conv2d(prev_layer, filter=W, strides=[1,1,1,1], padding='SAME')
        h_relu = tf.nn.relu(h_conv2d + b)
    return h_relu
...
def _avgpool(prev_layer):
    ...
    """
    out = tf.nn.avg_pool(prev_layer, ksize=[1,2,2,1], strides=[1,2,2,1],
